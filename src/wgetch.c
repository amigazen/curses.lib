...
static char *rcsid = "$Header: SRC:lib/curses/src/RCS/wgetch.c,v 1.4 92/06/10 23:44:59 sie Exp $";
...
wgetch(WINDOW *WinPtr)
{
  ...
  if(CursesType == CUST_CURSES) {
    if(GetchBufPos < GetchNChars)  /* If we still have some chars then return next */
      return (int)buffer[GetchBufPos++];

    /* Else read a new buffer */
    GetchBufPos = GetchNChars = 0;
    while(!GetchNChars) {
	...
	GetchNChars = RawKeyConvert(&ievent, buffer, RAWBUFSIZ, 0L);
	if(!GetchNChars)  /* If no characters then try again */
	  break;
	if(CursesFlags & CFLAG_ECHO)
	  for(i=0; i<GetchNChars; i++)
	    DoEcho(WinPtr, buffer[i]);
	/* Translate ANSI sequence if keypad set to TRUE */
	if(CursesFlags & CFLAG_KEYPAD) {
	  switch(GetchNChars) {
	  case 1:
	    GetchNChars = 0;  /* Translation will use up all chars */
	    if((CursesFlags & CFLAG_NLCR) && (buffer[0] == '\r'))
	      return (int)'\n';
	    return (int)*buffer;
	  case 2:    /* ARROW KEY */
	    GetchNChars = 0;  /* Translation will use up all chars */
    ...
    if((CursesFlags & CFLAG_NLCR) && (buffer[GetchBufPos] == '\r')) {
      GetchBufPos++;
      return (int)'\n';
    }
    return (int)buffer[GetchBufPos++];
  } else if(CursesType == ANSI_CURSES) {
    if((WinPtr->_nodelay) && (!WaitForChar(ifh, 10000L)))
      return ERR;		/* if nodelay and no char ready */
    read(0, &c, 1);
    if(CursesFlags & CFLAG_ECHO)
      DoEcho(WinPtr, c);
    if((CursesFlags & CFLAG_NLCR) && (c == '\r'))
      return (int)'\n';

    if(c == ESC && WaitForChar(ifh, 10000L)) { /* if ESC wait for [ */
      read(0, &c, 1);
      if(CursesFlags & CFLAG_ECHO)
	DoEcho(WinPtr, c);
      if(c != '[')
